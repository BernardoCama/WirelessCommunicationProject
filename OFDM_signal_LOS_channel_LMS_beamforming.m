...
%% LMS beamformer
N_training = round(length(chOut(:,1))/2);
[arrOut,w] = LMS_BF(Geometry, Pars, doas(:,1), chOut, waveform1(1:N_training,:));
...
% [arrOut,err,weights] = eqlms(arrOut,waveform1(1:100));
...
%arrOut1 = phaseCorrection2(arrOut, waveform1(1:100));
...
out = ofdmDemod1(arrOut);
out = reshape(out,[9*NumSymbols,1]);
dataInput1 = reshape(dataInput1,[9*NumSymbols,1]);
out = phaseCorrection2(out, dataInput1(1:100));

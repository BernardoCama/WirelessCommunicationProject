...
g = zeros(size(chOut_BF(:,1:n_training)));

for f = 1 : nfft_ch
    
    g(f, :) = Gradient_descent(chOut_BF(f, 1:n_training), dataInput1(f, 1:n_training), 0, 0, 100);
    
    
    
end

chOut_equal = zeros(size(chOut_BF, 1), size(chOut_BF, 2) + size(g, 2) - 1);
chOut_equal1 = zeros(size(chOut_BF, 1), size(chOut_BF, 2))

for f = 1 : nfft_ch
    chOut_equal(f, :) = conv(g(f, :), chOut_BF(f, :));
    chOut_equal1(f, :) = chOut_equal(f, 1 : size(chOut_BF, 2));
end

chOut_equal = chOut_equal1;

...
Y = zeros(size(chOut_BF(:,1:n_training)));
X = zeros(size(dataInput1(:,1:n_training)));
for f=1:nfft_ch
    
   % FFT of received sequence:
    Y(f,:) = fft(chOut_BF(f,1:n_training));

    % FFT of know sequence:
    X(f,:) = fft(dataInput1(f,1:n_training)); 
...
chOut_equal = mean((G)).*chOut_BF(:,1:end);
...
% With beamformer with equalization
figure;
chOut_equal = chOut_equal*exp(1i*pi/2);

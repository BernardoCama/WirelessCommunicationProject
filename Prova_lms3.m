...
g = Gradient_descent2( arrOut',  waveform1(1:n_training)', n_training, max_iter, g_len);

%arrOut_ = conv(g,arrOut);
arrOut_ = ifft(fft(g).*fft(arrOut));
...
% figure
% a = xcorr(bitInput1, dataOut_beam);
% plot (a)
% [value arg] = max(a);
% 
% ber = zeros (1,length(dataOut_beam));
% for i=1:length(dataOut_beam)
%     i;
%     dataOut_beam = circshift(dataOut_beam,1);
%     [numErrorsG_beam,ber(i)] = biterr(bitInput1(length(bitInput1)-length(dataOut_beam)+1:end),dataOut_beam(1:end));
%     
% end
% 
% [b,c] = min(ber)
